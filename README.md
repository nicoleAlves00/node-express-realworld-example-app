...
In line 69 in test/e2e_users_routes.js a test to get an user by ID is conducted. As the response is a Promise, chai.then.expect() is used. The test passes. However, there's a Error: Not found as the user is never returned, it is listed as user undefined meaning it gets the Promise but no the user.
